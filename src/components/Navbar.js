...
export const Navbar = () => {
  ...
  return (
    <div className='z-50 fixed top-0 flex justify-center w-full text-white font-bold'>
    ...
    <div className='border border-white/20 mt-8 backdrop-blur-3xl rounded-3xl hidden md:flex items-center justify-center p-2 max-w-[400px] mx-auto'>
        ...
        <ul className='flex flex-row p-2 space-x-8'>
          {navLinks.map((link,index)=>{
            return(
                <li key={index}>
                    <a className='transform hover:skew-x-12 hover:text-white/50 transition-all duration-300 ease-in-out'
          ...
          <ul className='flex flex-col items-center justify-center space-y-8 h-screen'>
            {navLinks.map((link,index)=>{
                return(
                    <li key={index}>
                        <a href={link.path} onClick={closeNav} className='text-5xl'>{link.title}</a>
                    </li>
                )
            })}
          </ul>
